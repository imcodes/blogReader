  ...
  <section class="section-sm">
    <div class="container">
        <div class="row no-gutter">
            ...
            @if (UserBlogCount($user->id) > 0)
            <div class="col-lg-4 col-sm-6 author-block">
                <div class="author-card text-center">

                  <img class="author-image" src="{{asset('storage/blogfiles/'.$user->profile_image)}}">

                  <h3 class="mb-2"><a href="{{route('author',str_replace(' ','-',$user->name)."-".$user->id)}}" class="post-title">{{$user->name}}</a></h3>
                  <p class="mb-3">{{$user->user_role}} &amp; {{$user->description}}</p>

                  <a class="post-count" href="author-single.html#post"><i class="ti-pencil-alt mr-2"></i><span class="text-primary">{{UserBlogCount($user->id)}}</span> Posts by this author</a>
